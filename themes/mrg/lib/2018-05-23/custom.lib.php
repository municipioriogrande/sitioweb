...
function MRG_make_file_hubspot_blog($url_post,$namefile)
{
	$name_file 	= $namefile;
...
function MRG_make_file_hubspot_blog_filter($url_post,$namefile,$hubspot_topics)
{
	$new_array = array();
	$name_file 	= $namefile;
	...
	if(is_array($data))
	{
		// Gereramos archivo x tag, dentro un array de tres elementos.
		...
		$name_file_2 	= dirname(__FILE__).'/json/data_hubspot_all.json';
		if(file_exists($name_file_2)){ @unlink($name_file_2); }
		$fp2 = fopen($name_file_2,"w");
		if($fp2 != false){ fwrite($fp2, json_encode($data)); fclose($fp2); }

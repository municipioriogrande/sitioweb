...
/* THISNEW MRG { */
function tg_header_gallery_slider_func($atts, $content) {
	extract(shortcode_atts(array(
		'gallery_id' => '',
		'size' => 'gallery_a',
		'height' => '500',
	...
	if(!empty($images_arr))
	{
		...
		foreach($images_arr as $key => $image)
		{
			...
			$return_html.= '<li class="slide" style="background-image: url('.$image_url[0].'); height:'.$height.'px;">';
			
			if(!empty($image_title))
			{
				$return_html.= '<div class="box">';
				$return_html.= '<h1>'.$image_title.'</h1>';
				$return_html.= '<p>'.$image_desc.'</p>';
				$return_html.= '</div>';
			}
...
add_shortcode('tg_header_gallery_slider', 'tg_header_gallery_slider_func');
/* } THISNEW MRG */

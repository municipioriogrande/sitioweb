...
?>
	...
	?>
		<div id="ppb_inline_<?php echo $key; ?>" data-shortcode="<?php echo $key; ?>" class="ppb_inline">
			...
			<?php
					...
					if($attr_item['type']=='html')
					{
			?>
			<label for="<?php echo $key; ?>_<?php echo $attr_name; ?>"><?php echo $attr_title; ?></label><span class="label_desc"><?php echo $attr_item['desc']; ?></span><br/>
            <textarea name="<?php echo $key; ?>_<?php echo $attr_name; ?>" id="<?php echo $key; ?>_<?php echo $attr_name; ?>" cols="" rows="7"></textarea>
			<br/><br/>
			<?php
					}
	...
	?>
			...
			jQuery(".pp_fancybox").click(function(){
				...
				foreach($ppb_shortcodes as $key => $ppb_shortcode)
				{
					foreach($ppb_shortcode['attr'] as $attr_name => $attr_item)
					{
						...
						{
						?>
						textareas.push("<?php echo $key; ?>_<?php echo $attr_name; ?>");
						//console.log('<?php echo $key; ?>_<?php echo $attr_name; ?>');
						<?php
						}
					}
				}
				...
				var a_href = jQuery(this).attr('href');
					a_href = a_href.replace("#ppb_inline_", "");
					
				var textareas_lenght = textareas.length;
				
				for (i = 0; i < textareas_lenght; i++)
				{
					if (textareas[i].indexOf(a_href) >= 0)
					{
						referencia.push(textareas[i]);
						//console.log(textareas[i]);
					}
				}
				
				var referencia_lenght = referencia.length;
				
				for (j = 0; j < referencia_lenght; j++)
				{
					//console.log('R: '+referencia[j]);*/
					var the_buttons 		= ["strong","em","link","block","del","ins","img","ul","ol","li","code"];
					var the_buttons_value 	= ["b","i","link","b-quote","del","ins","img","ul","ol","li","code"];
					var the_buttons_lenght = the_buttons.length;
					
					for (i = 0; i < the_buttons_lenght; i++)
					{
						valor = '#qt_'+referencia[j]+'_'+the_buttons[i];
						
						if(jQuery(valor).val()=='')
						{
							jQuery(valor).val(the_buttons_value[i]);
						}
					}
				}
			});

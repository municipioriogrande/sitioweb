...
function pp_shortcode_generator() {
  ...
  global $page_postmetas;
	if ( function_exists('add_meta_box') && isset($page_postmetas) && count($page_postmetas) > 0 ) {  
		...
		/* THISNEW { */
		add_meta_box( 'shortcode_metabox', 'Shortcode Options', 'pp_shortcode_generator_options', 'habitaciones', 'normal', 'high' );
		add_meta_box( 'shortcode_metabox', 'Shortcode Options', 'pp_shortcode_generator_options', 'slider', 'normal', 'high' );
		/* } THISNEW */
	}
}
...
function pp_shortcode_generator_options() {
	...
	$shortcodes = array(
		...
		'tg_header_gallery_slider' => array(
			'name' => 'Gallery Header Slider',
			'attr' => array(
				'gallery_id' => 'select',
			),
			'options' => $galleries_select,

...
<div id="page_content_wrapper" class="<?php if(!empty($pp_page_bg)) { ?>hasbg fix-height<?php } ?> <?php if(empty($page_menu_transparent)) { ?>notransparent<?php } ?> <?php if(!empty($pp_page_bg) && !empty($global_pp_topbar)) { ?>withtopbar<?php } ?>">
...
<?php
    ...
    {
    	$post_more_image = '';
    	if(has_post_thumbnail(get_the_ID(), 'gallery_2'))
    	{
    	    $post_more_image_id = get_post_thumbnail_id($prev_post->ID);
    	    $post_more_image = wp_get_attachment_image_src($post_more_image_id, 'gallery_2', true);
    	}
...
<div id="post_more_wrapper" class="hiding">
    <h5 class="widgettitle"><span><?php _e( 'More Story', THEMEDOMAIN ); ?></span><a href="javascript:;" id="post_more_close"><i class="fa fa-times"></i></a></h5><br class="clear"/>
    <?php
    ...
    <div class="content">
	    ...
	    <div class="post_detail">
		    <?php echo date(THEMEDATEFORMAT, strtotime($prev_post->post_date)); ?>&nbsp;-&nbsp;<?php echo date(THEMETIMEFORMAT, strtotime($prev_post->post_date)); ?>
		</div>

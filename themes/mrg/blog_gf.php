...
$query_string ="post_type=defaults&paged=$paged";
...
if (have_posts()) : while (have_posts()) : the_post();
	...
	$post_formato 			= !empty(get_post_meta(get_the_ID(), 'default_formato', true)) ? get_post_meta(get_the_ID(), 'default_formato', true) : 'c-1x1';
	...
	if($post_formato=='h-3x1'){ //horizontal 3x1
		$width = 747;
		$height = 420;
	}elseif($post_formato=='v-05x1'){ // Vertical 1/2x1
		$width = 199;
		$height = 420;
	}elseif($post_formato=='c-1x1'){ // Cuadrado 1x1
		$width = 519;
		$height = 420;
	}elseif($post_formato=='h-2x1'){ // Horizontal 2x1
		$width = 481;
		$height = 420;
	}
...
<div id="post-<?php the_ID(); ?>" <?php post_class(''.getCss($width).' '.getCss($height).''); ?> style="background-image: url(<?php echo $imagen; ?>);">
	...
	<div class="post_wrapper grid_layout">
		...
		?>
		    	    ...
		    	    <div class="post_img small static <?php echo getCss($width); ?> <?php echo getCss($height); ?>"></div>
		...
		?>
	    <div class="blog_grid_content <?php echo $post_format; ?>">
			<?php
				switch($post_format)
				{
					case 'quote':
